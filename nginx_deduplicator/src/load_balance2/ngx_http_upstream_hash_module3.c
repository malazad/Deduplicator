...
#include <ngx_core.h>
#include <ngx_http.h>
//#include "/Users/washik/Documents/nginx_dev/nginx/src/load_balance2/MyWrapper.h"
//#include "/Users/washik/Documents/nginx_dev/nginx/src/load_balance2/MyWrapper.cpp"
#include <stdio.h>
...
static ngx_uint_t SelectedBackEnd;
...
//static ngx_int_t ngx_http_upstream_get_hash_peer2(ngx_peer_connection_t *pc,
//    void *data, int *LSH);
...
ngx_module_t  ngx_http_upstream_hash_module3 = {
    NGX_MODULE_V1,
...
ngx_http_upstream_init_hash(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
    ...
    //ngx_log_error(NGX_LOG_DEBUG, cf->log,0,"checkpoint 1 ");
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 ");
    ...
    ngx_uint_t                       ii, nn;
    ngx_http_upstream_server_t      *server2;
    ngx_str_t hash_value;
    hash_value.data = r->unparsed_uri.data;
    hash_value.len = strlen((const char*)r->unparsed_uri.data);
    server2 = us->servers->elts;
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"number of servers 1: ... %d", us->servers->nelts );
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"hash value : ... %s", hash_value.data );
    char _value[hash_value.len -6];
    strncpy(_value, (const char*)hash_value.data + 6, hash_value.len -6);
    int hash_value_int;
    hash_value_int = atoi((const char*)_value);
    LSHash = hash_value_int;
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"hash value int: ... %d", hash_value_int );
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"hash value int (global): ... %d", LSHash );
    int test;
    test = foo(5);

    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"C Wrapper : ... %d", cpp_hello_world());
    //cpp_hello_world();

/*
    struct MyClass* c = newMyClass();
    MyClass_int_set(c, 3);
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"C Wrapper : ...1" );
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"C Wrapper : ... %d", MyClass_int_get2(c) );
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"C Wrapper : ... 2" );
    deleteMyClass(c);
*/
    ...
    for (nn = 0, ii = 0; ii < us->servers->nelts; ii++) {
        nn += server2[ii].naddrs;
        //struct sockaddr_in *addr_in = (struct sockaddr_in *) server2[ii].addrs->sockaddr;
        //char *s = inet_ntoa(addr_in->sin_addr);
        //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 server %d: %s : %d", ii, s, htons(addr_in->sin_port));
    }
    ...
    len = 0;
    len2 = 0;
    //ngx_int_t buf_count = 0;

    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 22 ");
    for (in = r->request_body->bufs; in; in = in->next) {
        len += ngx_buf_size(in->buf);
    }
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 len : %O", len);
    /*
    in = r->request_body->bufs;
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 buf : %s", in->buf->start);
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 buf : %s", in->buf->end);

    u_char *data_raw = in->buf->start;
    float data_int = atof((const char*)data_raw);

    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 raw data : %s", data_raw);
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 int data : %f", data_int);
    ...
    */
    ...
    //r->upstream->peer.get = ngx_http_upstream_get_hash_peer2;
    HashSize = 8;
    int BucketSize = (int) (1 << HashSize)/us->servers->nelts;
    SelectedBackEnd = (int)hash_value_int / BucketSize;
    r->upstream->peer.get = ngx_http_upstream_get_hash_peer;
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 get :.... %i", r->upstream->peer.get );
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 bucket size :.... %d", BucketSize);
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 2 selected backend :.... %d", SelectedBackEnd);

    hcf = ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_hash_module3);
...
ngx_http_upstream_get_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    //ngx_http_upstream_server_t      *server2;
    //server2 = hp->conf->servers->elts;
    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"number of servers 3: ... %d", hp->conf->servers->nelts );

    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"Global LSHash value : %d", LSHash);

    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 3 ");
    //ngx_http_request_t *r = pc->data;
    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 32 ");
    ...
    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 3 peers number: %i ", hp->rrp.peers->number);
    //char string[100];
    //strcpy(string, hp->rrp.peers->peer->sockaddr->sa_data[0]);
    //get_ip_str(hp->rrp.peers->peer->sockaddr->sa_data, string, 14);
    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 3 peers name: %s ", hp->rrp.peers->peer->name);
    //ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 3 peers server: %s ", hp->rrp.peers->peer->server);
...
ngx_http_upstream_init_chash(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
    ...
    //ngx_log_error(NGX_LOG_DEBUG, cf->log,0,"checkpoint 4 ");
    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    //ngx_log_error(NGX_LOG_DEBUG, r->connection->log,0,"checkpoint 5 ");
...
ngx_http_upstream_get_chash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    ngx_http_upstream_chash_points_t   *points;
    ngx_http_upstream_hash_srv_conf_t  *hcf;
    ngx_log_error(NGX_LOG_DEBUG, pc->log,0,"checkpoint 6 ");
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0,
...
ngx_http_upstream_hash_create_conf(ngx_conf_t *cf)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log,0,"checkpoint 7 ");
...
ngx_http_upstream_hash(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log,0,"checkpoint 8 ");
